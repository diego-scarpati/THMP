import * as jobService from "../services/jobDescription.services.js";

export const getAllJobDescriptions = async (req, res) => {
  try {
    const jobDescriptions = await jobService.getAllJobDescriptions();
...
export const getJobDescriptionById = async (req, res) => {
  const { id } = req.params;
  try {
    const jobDescription = await jobService.getJobDescriptionById(id);
...
export const createJobDescription = async (req, res) => {
  ...
  try {
    const newJobDescription = await jobService.createJobDescription(jobDescription);
